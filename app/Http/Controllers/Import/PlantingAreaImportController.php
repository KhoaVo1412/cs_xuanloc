...
class PlantingAreaImportController extends Controller
{
    public function importExcel(Request $request)
    {
        ...
        if (session()->has('temp_uploaded_pdfs')) {
            $tempFiles = session()->get('temp_uploaded_pdfs');

            foreach ($tempFiles as $filename) {
                $path = public_path('tmp_pdfs/' . $filename);
                if (file_exists($path)) {
                    unlink($path);
                }
            }

            session()->forget('temp_uploaded_pdfs'); // clear luôn session
        }
        ...
        try {
            ...
            if (!empty($import::$errors)) {
                ...
                foreach ($data as &$row) {
                    ...
                    if ($unit && $farm) {
                        $farmShort = app(PlantingAreaImport::class)->shortenFarmName($farm->farm_name);
                        $unitShort = app(PlantingAreaImport::class)->shortenFactoryName($unit->unit_name);
                ...
                return redirect()->route('add-excel')->with('preview_data', $data);
    ...
    public function add_excel()
    {
        ...
        if (empty($data)) {
            $tmpPath = public_path('tmp_pdfs');
            if (File::exists($tmpPath)) {
                File::cleanDirectory($tmpPath); // Xóa toàn bộ file PDF tạm
            }
            session()->forget('temp_uploaded_pdfs');
        }
    ...
    public function saveData(Request $request)
    {
        ...
        // Lấy mảng tên file đã upload tạm (nếu có)
        ...
        foreach ($data as &$row) {
            ...
            if ($pdfName && isset($uploadedPdfs[$pdfName])) {
                $tempPath = public_path('tmp_pdfs/' . $uploadedPdfs[$pdfName]);
                $finalPath = public_path('uploads/pdf/' . $uploadedPdfs[$pdfName]);

                if (file_exists($tempPath)) {
                    rename($tempPath, $finalPath); // di chuyển từ tmp → uploads
                }

                // Nếu cần cập nhật lại trong data
                $row['stored_pdf'] = $uploadedPdfs[$pdfName];
            }
        }
    ...
    public function uploadPdf(Request $request)
    {
        ...
        if ($request->hasFile('pdfs')) {
            ...
            $destinationPath = public_path('tmp_pdfs');
            ...
            foreach ($request->file('pdfs') as $file) {
                ...
                $existing = session()->get('temp_uploaded_pdfs', []);
                $merged = array_merge($existing, collect($uploaded)->pluck('stored')->toArray());
                session()->put('temp_uploaded_pdfs', $merged);
            }
