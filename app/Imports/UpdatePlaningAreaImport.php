...
class UpdatePlaningAreaImport implements ToCollection, WithHeadingRow, SkipsOnFailure, WithEvents
{
    ...
    public function savePlantingArea($rows)
    {
        foreach ($rows as $row) {
            ...
            $pdfPathToSave = $plantingArea->pdf ?? null;

            // Nếu có file PDF mới được upload
            if (isset($row['stored_pdf']) && !empty($row['stored_pdf'])) {
                $newFileName = $row['stored_pdf'];
                $newPath = 'uploads/pdf/' . $newFileName;

                // Tìm bản ghi theo mã lô (để tìm file cũ)
                $existingByMaLo = PlantingArea::where('ma_lo', $maLo)->first();

                if ($existingByMaLo && $existingByMaLo->pdf && $existingByMaLo->pdf !== $newPath) {
                    $oldFilePath = public_path($existingByMaLo->pdf);
                    if (file_exists($oldFilePath)) {
                        unlink($oldFilePath); // Xóa file PDF cũ
                    }
                }

                $pdfPathToSave = $newPath; // Gán file mới
            }
