    ...
    public function download()
    {
        ...
        if ($this->order->batches->isEmpty()) {
            ...
            foreach ($this->order->batches as $batch) {
                ...
                $plantationList = $batch->ingredients
                    ->flatMap(fn($ingredient) => $ingredient->plantingAreas)
                    ->map(fn($plantingArea) => $plantingArea->farm)
                    ->pluck('farm_name')
                    ->unique()
                    ->implode(', ');
                ...
                foreach ($batch->ingredients as $nl) {
                    foreach ($nl->plantingAreas as $planarea) {
                        ...
                        $geojson = json_decode($planarea->geo, true);
                        $xyList = [];
                        ...
                        foreach ($features as $feature) {
                            ...
                            if ($type === 'Polygon') {
                                $polygons = [$coordinates]; // Đưa về dạng giống MultiPolygon để xử lý chung
                            } elseif ($type === 'MultiPolygon') {
                                $polygons = $coordinates;
                            } else {
                                continue; // Bỏ qua nếu không phải Polygon/MultiPolygon
                            }

                            foreach ($polygons as $polygon) {
                                // polygon[0] là outer ring, polygon[1...] là inner rings (nếu có)
                                foreach ($polygon[0] as $point) {
                                    if (is_array($point) && count($point) === 2) {
                                        $x_raw = json_encode($point[0]);
                                        $y_raw = json_encode($point[1]);

                                        $x_raw = trim($x_raw, '"');
                                        $y_raw = trim($y_raw, '"');

                                        $x_parts = explode('.', $x_raw);
                                        $y_parts = explode('.', $y_raw);

                                        $x_full = $x_parts[0] . (isset($x_parts[1]) ? '.' . $x_parts[1] : '');
                                        $y_full = $y_parts[0] . (isset($y_parts[1]) ? '.' . $y_parts[1] : '');

                                        $xyList[] = "{$x_full}, {$y_full}";
                                    }
                                }
                            }
                        }
                        ...
                        // Ghi dữ liệu vào ô J{$row}
                        $newSheet->setCellValueExplicit("J{$row}", $xyString ?? '', DataType::TYPE_STRING);
                        $newSheet->getStyle("J{$row}")
                            ->getNumberFormat()
                            ->setFormatCode(NumberFormat::FORMAT_TEXT);

                        // Auto width cho cột J
                        $newSheet->getColumnDimension('J')->setAutoSize(true);
        ...
        }, "{$this->order->code}_DDS2.xlsx", [
            'Content-Type' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            'Content-Disposition' => 'attachment; filename="' . "{$this->order->code}_DDS2.xlsx" . '"',
